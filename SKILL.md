...
- **每种模式额外生成一张「极简传播封面」`00_cover.png`**（独立一张、不编号），专供小红书/公众号直接发：
  顶部小字栏目名 + 居中大图标 + 超大品牌名 + ≤24 字 slogan + 可选一句说明；背景跟随主题色，文字克制、唯一色彩爆点是图标/品牌色。
- 同时保留可编辑的 HTML 源文件。
...
- **v2**：晚秋简约风——纯排版 + 1px 细线，**无彩虹色、无渐变徽章、无重投影、无 3D 插画**。要点：
  - 品牌色从截图主色自动推导（降饱和、压暗成「高级感」色），金色 `#B08D45` 只做小面积强调；
  - 深浅主题随截图自动判定，每页 flex 撑满 1080×1440，不允许底部大片真空；
  - 概览图功能拆解为表格化两列（金色大编号 + 标题 + 说明），信息密度高于 v1 卡片；
  - 细节裁剪是「碎片」：只加细线框、**不加浏览器假地址条**，高度 < 300px 自动放大（最多 2 倍）；
...
1. **结构**：顶部小字栏目名 → 居中大图标（圆角方块框）→ 超大品牌名 → ≤24 字 slogan（利益点，不重复品牌）→ 可选 ≤20 字说明。无页脚（或仅极淡品牌水印）。
...
5. **slogan 上限 24 字**：写用户一眼懂的「好处」，不要功能词、不要重复品牌名。
...
- **纯文案模式**（已支持）：为每页生成对应主题的 3D 插画，见 `run_text_tutorial.py`。配置里 `cover` 与每个 `page` 都必须带 `visual_prompt`（3D 插画英文 prompt）；缺失会在文生图前**显式报错**并指出缺了哪几页（带 page_title/layout 便于定位），可加 `--no-validate` 跳过该校验（仅当你确实要纯文字页时）。
