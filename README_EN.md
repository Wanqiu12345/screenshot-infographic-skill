...
**② Plain-text mode** — give it just a topic (here, "Workflow vs Agent: what's the difference" as an example). It auto-structures the text and lays it out with expressive-type layouts; colors derived from the topic:

| v2 cover (SVG diagram encodes the content) | v2 content page (3-column comparison table) |
|---|---|
| ![WF cover](examples/wf_agent_v2_00_cover.png) | ![WF compare](examples/wf_agent_v2_03_difference.png) |
| ![WF flow](examples/wf_agent_v2_01_workflow.png) | ![WF choose](examples/wf_agent_v2_04_howtochoose.png) |

> Plain-text mode also has two styles: **v2 expressive layout** (above, `--style v2`: visual elements directly encode content — vertical chains = causality, dot grids = populations, timelines = evolution; every page fills 1080×1440 with zero whitespace, zero illustrations, zero cost) and **v1 magazine style** (default, 3D concept illustration + magazine layout, colors derived from topic, below):

| v1 cover (text-to-image 3D illustration) | v1 core-rules page (illustration fully visible, not cropped) |
|---|---|
...
### Mode 2: Plain text → infographics

**You send the AI**: a topic paragraph (e.g. "make me 5 science-popularization images about XX").
**The AI returns**: 1 cover + several content pages. Two styles available: **v2 expressive layout** (`--style v2`, pure typography with zero illustrations, visual elements directly encoding content) and **v1 magazine style** (default, each page auto-paired with a 3D concept illustration; colors derived from the topic).
...
# For the v2 expressive layout (pure typography, zero illustrations):
python run_text_tutorial.py my_text.json --style v2 --out output_text
# Reference example: examples/wf_agent_v2.json (Workflow vs Agent topic)

# Tip: for v1 magazine style, if text-to-image is slow, pre-generate the 3D illustrations in parallel, then reuse for render:
...
Plain-text mode supports auto-coloring by **category** or **named preset** — no longer hard-coded to cream white (v1 magazine style only; **v2 expressive layout needs zero config**, set `primary` in the json or let it auto-derive):
...
```mermaid
flowchart LR
    A[You send a paragraph of text] --> B[AI structure: split pages + colors]
    B --> C1{--style?}
    C1 -->|v1 default| D1[Text-to-image: 3D illustration per page] --> E1[Magazine layout + render]
    C1 -->|v2| D2[Expressive layouts: cover/chain/timeline/compare] --> E2[Pure typography render]
    E1 --> F[Set of 3:4 PNGs]
    E2 --> F
```
...
│   ├── text_v2.py                # Plain-text mode v2 "expressive layout" renderer (--style v2)
...
└── examples/                      # Examples & outputs (incl. README preview images)
    ...
    ├── jinan_drg_*.png            # Plain-text mode v1 example outputs
    ├── wf_agent_v2.json          # Plain-text mode v2 example config (Workflow vs Agent topic)
    └── wf_agent_v2_*.png          # Plain-text mode v2 example outputs
```
